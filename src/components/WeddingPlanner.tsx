...
export default function WeddingPlanner() {
  ...
  return <div className="space-y-6">
    ...
    <Card title="Budget Wedding" icon="💗">
      ...
      <div className="mt-4 max-h-[22rem] space-y-2 overflow-y-auto pr-1">{budget.map(item => <div key={item.id} className="flex flex-col gap-1 rounded-xl border border-forest/10 bg-white px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><b className="text-sm">{item.category}</b><span className="text-xs text-forest/60">Rencana {fmt(item.planned_amount)} • Aktual {fmt(item.actual_amount)}</span><button type="button" onClick={() => openEdit('Edit Budget', 'wedding_budget_items', item.id, [{ key: 'category', label: 'Kategori', value: item.category }, { key: 'planned_amount', label: 'Rencana', value: String(item.planned_amount), type: 'number' }, { key: 'actual_amount', label: 'Aktual', value: String(item.actual_amount), type: 'number' }], values => ({ category: values.category.trim(), planned_amount: Number(values.planned_amount) || 0, actual_amount: Number(values.actual_amount) || 0 }))} className="text-xs text-forest">Edit</button><button type="button" onClick={() => remove('wedding_budget_items', item.id)} className="text-xs text-red-500">Hapus</button></div>)}{!budget.length && <Empty text={`Belum ada budget. Total rencana: ${fmt(planned)}`} />}</div>
    </Card>


    <Card title="Kebutuhan Lainnya" icon="🛍️">
      <div className="rounded-2xl border border-forest/12 bg-gradient-to-br from-cream to-forest/5 p-4 sm:p-5">
        <div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-forest/10 text-xl">🛍️</span><div className="min-w-0"><h3 className="text-base font-black text-forest">Biaya di luar vendor</h3><p className="mt-1 text-xs leading-relaxed text-forest/60">Souvenir, transport, konsumsi tambahan, dan kebutuhan kecil lain otomatis mengurangi sisa budget wedding.</p></div></div>
        <div className="mt-4 flex items-center justify-between rounded-xl bg-gold px-3 py-3 shadow-sm shadow-gold/20"><span className="text-xs font-semibold text-cream/80">Total kebutuhan lain</span><span className="text-base font-black text-cream">-{fmt(needTotal)}</span></div>
      ...
      <div className="mt-4 max-h-[22rem] space-y-2 overflow-y-auto pr-1">{needs.map(item => <div key={item.id} className="flex flex-col gap-3 rounded-2xl border border-forest/10 bg-white/70 px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-red-100 text-sm">−</span><div className="min-w-0"><p className="truncate text-sm font-bold text-forest">{item.title}</p><p className="truncate text-xs text-forest/55">{item.note || 'Kebutuhan wedding'}</p></div></div><div className="flex items-center justify-between gap-3 border-t border-red-100 pt-2 sm:border-0 sm:pt-0"><span className="text-sm font-black text-red-500">-{fmt(item.amount)}</span><button type="button" onClick={() => remove('wedding_needs', item.id)} className="rounded-lg px-2 py-2 text-xs text-red-500 hover:bg-red-100">Hapus</button></div></div>)}{!needs.length && <Empty text="Belum ada kebutuhan lain." />}</div>
    </Card>
    ...
    <Card title="Timeline" icon="🗓️">
      ...
      <div className="mt-4 max-h-[22rem] space-y-2 overflow-y-auto pr-1">{timeline.map(item => <div key={item.id} className="flex items-center gap-3 rounded-xl border border-forest/10 bg-white px-3 py-3"><input type="checkbox" checked={item.done} onChange={() => toggle('wedding_timeline', item.id, item.done)} className="h-5 w-5 shrink-0 accent-pink-400" /><div className="min-w-0 flex-1"><p className={`truncate text-sm font-semibold ${item.done ? 'line-through text-forest/40' : ''}`}>{item.title}</p><p className="truncate text-xs text-forest/55">{item.event_date}{item.note && ` • ${item.note}`}</p></div><button type="button" onClick={() => openEdit('Edit Timeline', 'wedding_timeline', item.id, [{ key: 'title', label: 'Judul', value: item.title }, { key: 'event_date', label: 'Tanggal', value: item.event_date, type: 'date' }, { key: 'note', label: 'Catatan', value: item.note }], values => ({ title: values.title.trim(), event_date: values.event_date, note: values.note }))} className="shrink-0 text-xs text-forest">Edit</button><button type="button" onClick={() => remove('wedding_timeline', item.id)} className="shrink-0 text-xs text-red-500">Hapus</button></div>)}{!timeline.length && <Empty text="Belum ada timeline." />}</div>
    </Card>
