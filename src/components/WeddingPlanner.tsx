...
export default function WeddingPlanner() {
  ...
  return <div className="space-y-6">
    <Card title="Wedding Planner" icon="💍">
      <div className="relative overflow-hidden rounded-3xl border border-gold/25 bg-white/90 p-4 shadow-[0_16px_40px_rgba(216,79,138,0.10)] sm:p-6">
        ...
        <div className="relative">
          <p className="text-xs font-bold uppercase tracking-[.22em] text-forest/75">Wedding Dashboard</p>
    ...
    <Card title="Vendor" icon="🤝">
      ...
      <div className="mt-4 max-h-[22rem] space-y-2 overflow-y-auto pr-1">{vendors.map(item => <div key={item.id} className="flex flex-col gap-2 rounded-xl border border-forest/10 bg-white px-3 py-3 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><p className="truncate text-sm font-semibold">{item.name}</p><p className="truncate text-xs text-forest/75">{item.service || 'Vendor'} {item.contact && `• ${item.contact}`}</p></div><div className="flex items-center justify-between gap-3"><span className="text-xs">{fmt(item.paid_amount)} / {fmt(item.total_amount)}</span><button type="button" onClick={() => openEdit('Edit Vendor', 'wedding_vendors', item.id, [{ key: 'name', label: 'Nama vendor', value: item.name }, { key: 'service', label: 'Layanan', value: item.service }, { key: 'contact', label: 'Kontak', value: item.contact }, { key: 'total_amount', label: 'Total harga', value: String(item.total_amount), type: 'number' }, { key: 'paid_amount', label: 'Sudah dibayar', value: String(item.paid_amount), type: 'number' }], values => ({ name: values.name.trim(), service: values.service, contact: values.contact, total_amount: Number(values.total_amount) || 0, paid_amount: Number(values.paid_amount) || 0 }))} className="text-xs text-forest">Edit</button><button type="button" onClick={() => remove('wedding_vendors', item.id)} className="text-xs text-red-500">Hapus</button></div></div>)}{!vendors.length && <Empty text="Belum ada vendor." />}</div>
    </Card>

    <Card title="Budget Wedding" icon="💗">
      <div className="rounded-2xl border border-forest/10 bg-white/90 p-4 sm:p-5"><div className="mb-4 flex items-start justify-between gap-3"><div><p className="text-sm font-black text-forest">Tambah anggaran</p><p className="mt-0.5 text-xs text-forest/75">Rencana dan biaya aktual per kategori.</p></div><div className="shrink-0 rounded-xl border border-forest/15 bg-cream px-3 py-2 text-right"><p className="text-[10px] font-semibold text-forest/75">Total rencana</p><p className="mt-0.5 text-sm font-black text-forest">{fmt(planned)}</p></div></div><div className="grid gap-3 sm:grid-cols-2"><Input label="Kategori budget" value={budgetForm.category} onChange={e => setBudgetForm({ ...budgetForm, category: e.target.value })} placeholder="Contoh: Katering" className="sm:col-span-2" /><CurrencyInput label="Rencana biaya" value={budgetForm.planned} onValueChange={planned => setBudgetForm({ ...budgetForm, planned })} placeholder="0" /><CurrencyInput label="Biaya aktual" value={budgetForm.actual} onValueChange={actual => setBudgetForm({ ...budgetForm, actual })} placeholder="0" /></div><Button type="button" className="mt-4 w-full sm:w-auto sm:px-7" onClick={() => budgetForm.category.trim() && add('wedding_budget_items', { category: budgetForm.category.trim(), planned_amount: Number(budgetForm.planned) || 0, actual_amount: Number(budgetForm.actual) || 0 }, () => setBudgetForm({ category: '', planned: '', actual: '' }))}>+ Tambah Kategori</Button></div>
      <div className="mt-3 max-h-[22rem] divide-y divide-white/5 overflow-y-auto rounded-2xl border border-forest/10 bg-white/90 px-3">{budget.map(item => <div key={item.id} className="flex items-center gap-2 py-3"><div className="min-w-0 flex-1"><b className="block truncate text-sm text-forest">{item.category}</b><span className="mt-0.5 block text-[11px] text-forest/75">Rencana {fmt(item.planned_amount)} · Aktual {fmt(item.actual_amount)}</span></div><button type="button" onClick={() => openEdit('Edit Budget', 'wedding_budget_items', item.id, [{ key: 'category', label: 'Kategori', value: item.category }, { key: 'planned_amount', label: 'Rencana', value: String(item.planned_amount), type: 'number' }, { key: 'actual_amount', label: 'Aktual', value: String(item.actual_amount), type: 'number' }], values => ({ category: values.category.trim(), planned_amount: Number(values.planned_amount) || 0, actual_amount: Number(values.actual_amount) || 0 }))} className="shrink-0 rounded-lg px-2 py-2 text-xs text-forest/80 hover:bg-white/5">Edit</button><button type="button" onClick={() => remove('wedding_budget_items', item.id)} className="shrink-0 rounded-lg px-2 py-2 text-xs text-red-500 hover:bg-red-50">Hapus</button></div>)}{!budget.length && <Empty text={`Belum ada budget. Total rencana: ${fmt(planned)}`} />}</div>
    </Card>


    <Card title="Kebutuhan Lainnya" icon="🛍️">
      <div className="rounded-2xl border border-forest/10 bg-white/90 p-4 sm:p-5">
        <div className="flex items-start gap-3"><span className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-gold/15 text-xl">🛍️</span><div className="min-w-0"><h3 className="text-base font-black text-forest">Biaya di luar vendor</h3><p className="mt-1 text-xs leading-relaxed text-forest/75">Souvenir, transport, konsumsi tambahan, dan kebutuhan kecil lain otomatis mengurangi sisa budget wedding.</p></div></div>
        <div className="mt-4 flex items-center justify-between rounded-xl border border-forest/15 bg-cream px-3 py-3"><span className="text-xs font-semibold text-forest/75">Total kebutuhan lain</span><span className="text-base font-black text-forest">-{fmt(needTotal)}</span></div>
      ...
      <div className="mt-3 grid max-h-[22rem] gap-2 overflow-y-auto pr-1 sm:grid-cols-2">{needs.map(item => <article key={item.id} className="rounded-xl border border-gold/20 bg-white/90 p-3"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="truncate text-sm font-bold text-forest">{item.title}</p><p className="mt-0.5 truncate text-[11px] text-forest/75">{item.note || 'Tanpa catatan'}</p></div><span className="shrink-0 text-sm font-bold text-forest">{fmt(item.amount)}</span></div><div className="mt-2 flex justify-end gap-1"><button type="button" onClick={() => openEdit('Edit Kebutuhan', 'wedding_needs', item.id, [{ key: 'title', label: 'Nama kebutuhan', value: item.title }, { key: 'amount', label: 'Nominal', value: String(item.amount), type: 'number' }, { key: 'note', label: 'Catatan', value: item.note }], values => ({ title: values.title.trim(), amount: Number(values.amount) || 0, note: values.note }))} className="rounded-md px-2 py-1 text-[11px] font-semibold text-forest/80 hover:bg-white/5">Edit</button><button type="button" onClick={() => remove('wedding_needs', item.id)} className="rounded-md px-2 py-1 text-[11px] font-semibold text-red-500 hover:bg-red-50">Hapus</button></div></article>)}{!needs.length && <div className="sm:col-span-2"><Empty text="Belum ada kebutuhan lain." /></div>}</div>
    </Card>
    ...
    <Card title="Timeline" icon="🗓️">
      ...
      <div className="mt-4 max-h-[25rem] space-y-0 overflow-y-auto pr-1">{timeline.map((item, index) => <div key={item.id} className="relative flex gap-3 pb-3 last:pb-0"><div className="flex w-10 shrink-0 flex-col items-center"><span className={`grid h-10 w-10 place-items-center rounded-xl border text-center leading-none ${item.done ? 'border-emerald-300/30 bg-emerald-50 text-emerald-500' : 'border-forest/25 bg-cream text-forest'}`}><b className="text-sm">{new Date(`${item.event_date}T00:00:00`).getDate()}</b><small className="-mt-1 text-[8px] font-bold uppercase">{new Date(`${item.event_date}T00:00:00`).toLocaleDateString('id-ID', { month: 'short' })}</small><small className="text-[7px] font-medium opacity-70">{new Date(`${item.event_date}T00:00:00`).getFullYear()}</small></span>{index < timeline.length - 1 && <span className="mt-1 w-px flex-1 bg-forest/15" />}</div><article className={`min-w-0 flex-1 rounded-2xl border p-3 ${item.done ? 'border-forest/10 bg-white/50' : 'border-forest/15 bg-white/90'}`}><div className="flex items-start gap-2"><input type="checkbox" checked={item.done} onChange={() => toggle('wedding_timeline', item.id, item.done)} className="mt-0.5 h-4 w-4 shrink-0 accent-pink-400" /><div className="min-w-0 flex-1"><div className="flex items-start justify-between gap-2"><p className={`min-w-0 flex-1 whitespace-normal break-words text-sm font-bold leading-snug ${item.done ? 'text-forest/65 line-through' : 'text-forest'}`}>{item.title}</p><span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-bold ${item.done ? 'bg-forest/10 text-forest/75' : 'bg-forest/10 text-forest'}`}>{relativeDay(item.event_date)}</span></div><p className="mt-1 text-[11px] text-forest/75">{new Date(`${item.event_date}T00:00:00`).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' })}</p>{item.note && <p className="mt-2 rounded-lg border border-white/5 bg-cream px-2.5 py-2 text-xs leading-relaxed text-forest/80">{item.note}</p>}</div></div><div className="mt-2 flex justify-end gap-1 border-t border-white/5 pt-2"><button type="button" onClick={() => openEdit('Edit Timeline', 'wedding_timeline', item.id, [{ key: 'title', label: 'Judul', value: item.title }, { key: 'event_date', label: 'Tanggal', value: item.event_date, type: 'date' }, { key: 'note', label: 'Catatan', value: item.note }], values => ({ title: values.title.trim(), event_date: values.event_date, note: values.note }))} className="rounded-md px-2 py-1 text-[11px] text-forest/80 hover:bg-white/5">Edit</button><button type="button" onClick={() => remove('wedding_timeline', item.id)} className="rounded-md px-2 py-1 text-[11px] text-red-500 hover:bg-red-50">Hapus</button></div></article></div>)}{!timeline.length && <Empty text="Belum ada timeline." />}</div>
    </Card>
...
function Stat({ value, label, small }: { value: string; label: string; small?: boolean }) { return <div className="min-w-0 rounded-xl bg-cream/70 p-3"><p className={`${small ? 'text-sm' : 'text-lg'} truncate font-bold text-forest`}>{value}</p><p className="text-[10px] text-forest/75">{label}</p></div> }
function Empty({ text }: { text: string }) { return <p className="py-5 text-center text-sm text-forest/65">{text}</p> }
